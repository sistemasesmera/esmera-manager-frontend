...
export default function TableCourses() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        // Enviar searchTerm solo si no está vacío
        if (searchTerm.trim() !== "") {
        ...
        // TotalPages se calcula correctamente según la API
  ...
  const toggleCourseStatus = async (courseId: string, newStatus: boolean) => {
    // Evitar doble click
    ...
    try {
      await axiosInstance.patch(`courses/${courseId}/status`, {
        isActive: newStatus,
      });
      ...
      setCourses((prev) =>
        prev.map((course) =>
          course.id === courseId ? { ...course, isActive: newStatus } : course
        )
      );
    } catch (error) {
      console.error(error);
                  ...
                  <TableRow key={index}>
                    ...
                    >
                      <Switch
                        ...
                        onChange={() =>
                          toggleCourseStatus(item.id, !item.isActive)
                        }
                      />
                    </TableCell>
                    <TableCell
                    ...
                    >
                      <InformationCourseModal
                        ...
                        onUpdate={(updatedCourse) => {
                          setCourses((prev) =>
                            prev.map((a) =>
                              a.id === updatedCourse.id ? updatedCourse : a
                            )
                          );
                        }}
                      />
